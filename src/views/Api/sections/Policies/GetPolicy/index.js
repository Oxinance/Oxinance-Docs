...
const GetPolicy = ({spacing}) => {
        ...
        if (selectedLanguage === "Node JS") {
            return (
                <>
                    ...
                    <p style={{color: "#F5FBFF", fontFamily: "JetBrainsMono-Medium", fontSize: 13}}><span style={{color: "#697386"}}>2</span> <Token>const</Token> Policy = require(<String>&quot;oxinance/dist/policies/enum&quot;</String>)<Comment>;</Comment></p>
                    <p style={{color: "#F5FBFF", fontFamily: "JetBrainsMono-Medium", fontSize: 13}}><span style={{color: "#697386"}}>3</span></p>
                    <p style={{color: "#F5FBFF", fontFamily: "JetBrainsMono-Medium", fontSize: 13}}><span style={{color: "#697386"}}>4</span> oxinance<Comment>.</Comment>publicKey = <String>&quot;{examplePublicKey}&quot;</String><Comment>;</Comment></p>
                    <p style={{color: "#F5FBFF", fontFamily: "JetBrainsMono-Medium", fontSize: 13}}><span style={{color: "#697386"}}>5</span></p>
                    <p style={{color: "#F5FBFF", fontFamily: "JetBrainsMono-Medium", fontSize: 13}}><span style={{color: "#697386"}}>6</span> <Token>const</Token> response = <Token>await</Token> oxinance<Comment>.</Comment>Policies<Comment>.</Comment>retrieve(Policies.PRIVACY_POLICY)<Comment>;</Comment></p>
                </>
            )
        }
            ...
            return (
                <>
                    ...
                    <p style={{color: "#F5FBFF", fontFamily: "JetBrainsMono-Medium", fontSize: 13}}><span style={{color: "#697386"}}>5</span> <Token>const</Token> response = <Token>await</Token> axios.get(<String>&quot;{endpoint}&quot;</String>);</p>
                </>
            )
    ...
    return (
        <Grid id={"api-policies-get"} style={{backgroundColor: "white", paddingTop: 80, paddingBottom: 80}} container px={spacing} columnSpacing={10}>
            <Grid item xs={12} lg={6}>
                <p style={{color: "#2A2F45", fontWeight: 500, fontSize: 24, marginBottom: 10}}>Get Policy</p>
                <p style={{fontSize: 14, color: "#4F566B"}}>Retrieves the Policy specified in the URL.</p>
                <br/>
                ...
                <Divider/>
                <p style={{fontSize: 14, color: "#4F566B"}}>Returns a Policy object if it exists.</p>
            </Grid>
            <Grid item xs={12} lg={6}>
                ...
                <EndpointsCode title={"RESPONSE"} style={{marginTop: 10}}>
                    <pre style={{fontFamily: "JetBrainsMono-Medium", color: "#697386", fontSize: 13}} dangerouslySetInnerHTML={{__html: jsonSyntaxHighlight(getPolicyResponse)}}/>
                </EndpointsCode>
            </Grid>
        </Grid>
    )
