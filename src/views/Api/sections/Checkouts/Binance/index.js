...
const endpoint = "/v1/checkouts/binance"
        ...
        if (selectedLanguage === "Node JS") {
            return (
                <>
                    ...
                    <p style={{color: "#F5FBFF", fontFamily: "JetBrainsMono-Medium", fontSize: 13}}><span style={{color: "#697386"}}>6</span> <Token>const</Token> response = <Token>await</Token> oxinance<Comment>.</Comment>Checkouts<Comment>.</Comment>generateBinanceCheckout()<Comment>;</Comment></p>
                </>
            )
        }
        ...
        <Grid id={"api-checkout-binance"} style={{backgroundColor: "white", paddingTop: 80, paddingBottom: 80}} container px={spacing} columnSpacing={10}>
            ...
            <Grid item xs={12} lg={6}>
                ...
                <EndpointsCode title={"RESPONSE"} style={{marginTop: 10}}>
                    <pre style={{fontFamily: "JetBrainsMono-Medium", color: "#697386", fontSize: 13}} dangerouslySetInnerHTML={{__html: jsonSyntaxHighlight(binanceResponse)}}/>
                </EndpointsCode>
            </Grid>
        </Grid>
    )
