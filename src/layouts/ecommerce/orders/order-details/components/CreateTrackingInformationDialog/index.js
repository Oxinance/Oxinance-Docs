...
export default function CreateTrackingInformationDialog({open, order, successCallback, closeFunction}) {
    console.log(order.tracking_information)
    const projects = useSelector(state => state.projects);
    const [error, setError] = useState({display: false, message: null})
    const [loading, setLoading] = useState(false)
    const [data, setData] = useState(order.tracking_information ? {...order.tracking_information} : {
        company: "",
        code: "",
        url: ""
    })
    ...
    const handleCreate = () => {
        ...
        OrdersClient.createTrackingInformation((projects.isLive ? "live" : "sandbox"), projects.selectedProject.id, order.id, data, callback, errorCallback)
    }
